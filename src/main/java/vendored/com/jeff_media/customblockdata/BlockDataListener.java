...
final class BlockDataListener implements Listener {
  ...
  private boolean callEvent(Block block, Event bukkitEvent) {
    if (!CustomBlockData.hasCustomBlockData(block, plugin)
        || CustomBlockData.isProtected(block, plugin)) {
      return false;
    }

    CustomBlockDataRemoveEvent cbdEvent =
        new CustomBlockDataRemoveEvent(plugin, block, bukkitEvent);
    ...
    blocks.stream()
        ...
        .forEach(
            block -> {
              ...
              CustomBlockDataMoveEvent moveEvent =
                  new CustomBlockDataMoveEvent(plugin, block, destinationBlock, bukkitEvent);
