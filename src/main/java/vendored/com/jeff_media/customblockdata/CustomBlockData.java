...
package vendored.com.jeff_media.customblockdata;
...
/**
 * Represents a {@link PersistentDataContainer} for a specific {@link Block}. Also provides some
 * static utility methods that can be used on every PersistentDataContainer.
 *
 * <p>By default, and for backward compatibility reasons, data stored inside blocks is independent
 * of the underlying block. That means: if you store some data inside a dirt block, and that block
 * is now pushed by a piston, then the information will still reside in the old block's location.
 * <b>You can of course also make CustomBockData automatically take care of those situations</b>, so
 * that CustomBlockData will always be updated on certain Bukkit Events like BlockBreakEvent,
...
public class CustomBlockData implements PersistentDataContainer {
  ...
  private static final char[] DEFAULT_PACKAGE =
      new char[] {
        'c', 'o', 'm', '.', 'j', 'e', 'f', 'f', '_', 'm', 'e', 'd', 'i', 'a', '.', 'c', 'u', 's',
        't', 'o', 'm', 'b', 'l', 'o', 'c', 'k', 'd', 'a', 't', 'a'
      };

  /**
   * Set of "dirty block positions", that is blocks that have been modified and need to be saved to
   * the chunk
   */
  ...
  /**
   * Whether WorldInfo#getMinHeight() method exists. In some very specific versions, it's directly
   * declared in World.
   */
  ...
  /**
   * The Map.Entry containing the UUID of the block and its BlockVector for usage with {@link
   * #DIRTY_BLOCKS}
   */
        ...
        plugin
            ...
            .warning(
                "Nag author(s) "
                    + " of plugin "
                    + plugin.getName()
                    + " for not relocating the CustomBlockData package.");
   ...
   *
   * <p>If the plugin is disabled, this method will do nothing, to prevent the
  ...
  static void setDirty(Plugin plugin, Map.Entry<UUID, BlockVector> blockEntry) {
    if (!plugin.isEnabled()) // checks if the plugin is disabled to prevent the
      // IllegalPluginAccessException
      return;
  ...
  /**
   * Gets a String-based {@link NamespacedKey} that consists of the block's relative coordinates
   * within its chunk
   *
   * @param block block
   * @return NamespacedKey consisting of the block's relative coordinates within its chunk
   */
  @NotNull static String getKey(@NotNull final Block block) {
  ...
  @Nullable static Block getBlockFromKey(final NamespacedKey key, final Chunk chunk) {
  ...
  /**
   * Returns the given {@link World}'s minimum build height, or 0 if not supported in this Bukkit
   * version
   */
  ...
  public static boolean hasCustomBlockData(Block block, Plugin plugin) {
    return block
        ...
        .has(getKey(plugin, block), PersistentDataType.TAG_CONTAINER);
  }

  /**
   * Get if the given Block's CustomBlockData is protected. Protected CustomBlockData will not be
   * changed by any Bukkit Events
   *
   * @return true if the Block's CustomBlockData is protected, false if it doesn't have any
   *     CustomBlockData or it's not protected
  ...
  /**
   * Starts to listen and manage block-related events such as {@link BlockBreakEvent}. By default,
   * CustomBlockData is "stateless". That means: when you add data to a block, and now a player
   * breaks the block, the data will still reside at the original block location. This is to ensure
   * that you always have full control about what data is saved at which location.
   *
   * <p>If you do not want to handle this yourself, you can instead let CustomBlockData handle those
   * events by calling this method once. It will then listen to the common events itself, and
   * automatically remove/update CustomBlockData.
   *
   * <p>Block changes made using the Bukkit API (e.g. {@link Block#setType(Material)}) or using a
   * plugin like WorldEdit will <b>not</b> be registered by this (but pull requests are welcome, of
   * course)
   *
  ...
  /**
   * Returns a Set of all blocks in this chunk containing Custom Block Data created by the given
   * plugin
   *
   * @param plugin Plugin
   * @param chunk Chunk
   * @return A Set containing all blocks in this chunk containing Custom Block Data created by the
   *     given plugin
   */
  @NotNull public static Set<Block> getBlocksWithCustomData(final Plugin plugin, final Chunk chunk) {
    final NamespacedKey dummy = new NamespacedKey(plugin, "dummy");
    return getBlocksWithCustomData(chunk, dummy);
  }

  /**
   * Returns a {@link Set} of all blocks in this {@link Chunk} containing Custom Block Data matching
   * the given {@link NamespacedKey}'s namespace
   *
   * @param namespace Namespace
   * @param chunk Chunk
   * @return A {@link Set} containing all blocks in this chunk containing Custom Block Data created
   *     by the given plugin
   */
  @NotNull private static Set<Block> getBlocksWithCustomData(
      final @NotNull Chunk chunk, final @NotNull NamespacedKey namespace) {
    final PersistentDataContainer chunkPDC = chunk.getPersistentDataContainer();
    return chunkPDC.getKeys().stream()
        .filter(key -> key.getNamespace().equals(namespace.getNamespace()))
  ...
  /**
   * Returns a {@link Set} of all blocks in this {@link Chunk} containing Custom Block Data created
   * by the given plugin
   *
   * @param namespace Namespace
   * @param chunk Chunk
   ...
   *     by the given plugin
   */
  @NotNull public static Set<Block> getBlocksWithCustomData(final String namespace, final Chunk chunk) {
    final NamespacedKey dummy = new NamespacedKey(namespace, "dummy");
    return getBlocksWithCustomData(chunk, dummy);
  }

  /**
   * Gets the proper primitive {@link PersistentDataType} for the given {@link NamespacedKey} in the
   * given {@link PersistentDataContainer}
   *
   * @return The primitive PersistentDataType for the given key, or null if the key doesn't exist
   */
  public static PersistentDataType<?, ?> getDataType(
   ...
   */
  @NotNull private PersistentDataContainer getPersistentDataContainer() {
    final PersistentDataContainer chunkPDC = chunk.getPersistentDataContainer();
  ...
  /**
   * Gets whether this CustomBlockData is protected. Protected CustomBlockData will not be changed
   * by any Bukkit Events
   *
   * @see #registerListener(Plugin)
   */
  ...
  /**
   * Sets whether this CustomBlockData is protected. Protected CustomBlockData will not be changed
   * by any Bukkit Events
   *
   * @see #registerListener(Plugin)
   */
  ...
  /**
   * Copies all data to another block. Data already present in the destination block will keep
   * intact, unless it gets overwritten by identically named keys. Data in the source block won't be
   * changed.
   */
  ...
  private static final class DataType {
    private static final PersistentDataType<Byte, Boolean> BOOLEAN =
        new PersistentDataType<>() {
          @NotNull @Override
          ...
          public Byte toPrimitive(
              @NotNull Boolean complex, @NotNull PersistentDataAdapterContext context) {
