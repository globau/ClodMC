...
package vendored.com.jeff_media.customblockdata.events;
 ...
 *
 * <p>This event gets called during the underlying Bukkit Event's MONITOR listening phase. If the
 * Bukkit Event was already cancelled, this event will not be called.
 *
...
public class CustomBlockDataEvent extends Event implements Cancellable {
  ...
  protected CustomBlockDataEvent(
  ...
  /**
   * Gets the underlying Bukkit Event that has caused this event to be called. The Bukkit Event is
   * currently listened on in MONITOR priority.
   */
  ...
  /**
   * Sets the cancellation status of this event. If the event is cancelled, the CustomBlockData will
   * not be removed, changed or moved.
   */
  @Override
  ...
  public enum Reason {
    ...
    /**
     * Represents a block being replaced by a new block (for example STONE being placed into a
     * TALL_GRASS)
     *
     * @see BlockPlaceEvent
    ...
    /**
     * Represents a block being changed by an entity. An {@link EntityChangeBlockEvent} will only
     * trigger removal of CustomBlockData when the block's material changes.
     *
     * <p>Example: When a player steps on REDSTONE_ORE, an EntityChangeBlockEvent is called because
     * the BlockState's "lit" tag changes from false to true. However, this will not lead to removal
     * of CustomBlockData because the block's material is still REDSTONE_ORE.
     */
    ENTITY_CHANGE_BLOCK(EntityChangeBlockEvent.class),
    /**
     * Represents a block being destroyed by melting, etc. A {@link BlockFadeEvent} will only
     * trigger removal of CustomBlockData when the block's material changes. The event will not be
     * called for fire burning out.
     *
     * @see BlockFadeEvent
     */
    FADE(BlockFadeEvent.class),
    /**
     * Represents a block being changed by a structure (Sapling -> Tree, Mushroom -> Huge Mushroom),
     * naturally or using bonemeal.
     *
    ...
    /**
     * Represents leaves decaying. This is currently not called because of performance reasons. In
     * future versions, there will be a method to enable listening to this.
     *
